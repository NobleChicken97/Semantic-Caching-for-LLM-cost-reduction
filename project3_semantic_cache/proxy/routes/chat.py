...
async def chat_completions(body: ChatCompletionRequest, request: Request):
    ...
    if bypass:
        ...
        raw_resp, upstream_latency = await forward_to_llm(
    ...
    # --- Cache lookup (exact string match) ---
    cached = lookup(prompt)
    if cached is not None:
        cached["response"]["cache_metadata"] = CacheMetadata(
            outcome="HIT", similarity_score=1.0
        ).model_dump()
        # latency for a hit is near-zero (just the DB lookup)
        log_request(
            prompt_text=prompt,
            outcome="HIT",
            latency_ms=0.0,
