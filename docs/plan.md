...
| **8** Dashboard redesign | Single-file visual + insights overhaul of `/dashboard` (no backend change) | ✅ Shipped & live 2026-09-04 (Phase 8 → Watermelon re-skin → statement/ledger → bento Overview + hand-drawn SVG viz + rail + sharp boxes, all screenshot-verified) | dark-first toggle, speedup gauge + trend + auto-tune evidence, anime.js motion with reduced-motion fallback |
| **9** Semantic-trust fixes | Eval/prod skew (prefix inflation) + entity-substitution false hits found by deep battery | ✅ Shipped & live 2026-09-04 (Fix A message-only embeddings; Fix B two-signal veto with calibrated 0.2 template gate; Fix C rejected with evidence) | live re-measure: R=0.9375, P=0.82, spotlight all-MISS, session 10/11 |
| **10** Combinatorial battery + veto round 2 | 44-country + persons + facts + dates matrices, boring phrases, order/negation, encoding, conc20, drift; 5-signal veto (template-gated Fix C, negation/antonym + number guards) | ✅ Closed 2026-09-04 | combinatorial recall 16/16 (1.0), precision 0.9851, STRICT 48/48; Fix C recall hole (see-you sim 0.96) fixed via shared-skeleton gate |
| **10** Combinatorial battery | 44-country + persons + facts + dates matrices, boring phrases, order/negation, encoding, conc20, drift | ✅ Shipped & live 2026-09-04 (`data/phase10probes.json`, `scripts/test_phase10.py`, `tests/test_lifecycle.py`; 172 tests green) | combinatorial P=0.94/R=1.0, conc20 perfect, drift 0.0000; 7 findings reported, no prod fixes (battery-only pass) |
...
- **Workflow layout — Decided 2026-09-04:** keep 3 files (`ci.yml`, `deploy.yml`, `live-monitor.yml`). Merging into one file is possible (gated jobs) but adds PR noise and secret-surface risk; separate triggers stay independent and re-runnable.
